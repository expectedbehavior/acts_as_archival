...
class ArchivalTest < ActiveSupport::TestCase
  ...
  test "including archival throws exceptions if the correct columns aren't in place" do
    assert_raises(Archival::MissingArchivalColumnError) { Kitty.create!(:name => "foo-foo")}
    assert_raises(Archival::MissingArchivalColumnError) { Puppy.create!(:name => "rover")}
  end
